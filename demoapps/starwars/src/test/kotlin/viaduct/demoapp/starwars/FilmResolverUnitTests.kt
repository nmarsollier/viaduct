...
class FilmResolverUnitTests : DefaultAbstractResolverTestBase() {
    ...
    fun `FilmCharacterCountSummaryResolver counts characters`() =
        runBlockingTest {
            ...
            val luke = Character.Builder(context).name("Luke Skywalker").build()
            val leia = Character.Builder(context).name("Leia Organa").build()
            val han = Character.Builder(context).name("Han Solo").build()

            val result = runFieldResolver(
                resolver = resolver,
                objectValue = Film.Builder(context)
                    .title("Star Wars: A New Hope")
                    .characters(listOf(luke, leia, han))
                    .build(),
            )

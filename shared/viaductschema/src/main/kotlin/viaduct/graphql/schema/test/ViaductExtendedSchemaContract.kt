...
/** A set of unit tests for ViaductExtendedSchema implementations, with emphasis on
 *  catching error cases (non-error cases are tested pretty well in the "full"
 *  tests).
 */
interface ViaductExtendedSchemaContract {
    companion object {
        private fun ViaductExtendedSchema.withType(
            type: String,
            block: (ViaductExtendedSchema.TypeDef) -> Unit
        ) = block(this.types[type] ?: throw IllegalArgumentException("Unknown type $type"))

        private fun ViaductExtendedSchema.withExtensions(
            type: String,
            block: (Iterable<ViaductExtendedSchema.Extension<*, *>>) -> Unit
        ) = block(
            (this.types[type] as? ViaductExtendedSchema.HasExtensions<*, *>)?.extensions
                ?: throw IllegalArgumentException("Unknown on non-extensions type $type")
        )

        private fun ViaductExtendedSchema.withField(
            type: String,
            field: String,
            block: (ViaductExtendedSchema.Field) -> Unit
        ) = block((this.types[type]!! as ViaductExtendedSchema.Record).field(field)!!)

        private fun ViaductExtendedSchema.withArg(
            type: String,
            field: String,
            arg: String,
            block: (ViaductExtendedSchema.FieldArg) -> Unit
        ) = block((this.types[type]!! as ViaductExtendedSchema.Record).field(field)!!.args.find { it.name == arg }!!)

        private fun ViaductExtendedSchema.withEnumValue(
            enum: String,
            value: String,
            block: (ViaductExtendedSchema.EnumValue) -> Unit
        ) = block((this.types[enum]!! as ViaductExtendedSchema.Enum).value(value)!!)
    ...
    fun makeSchema(schema: String): ViaductExtendedSchema
    ...
    fun `Test the fields getter that takes a path`() {
        makeSchema(
            ...
            val query = this.types["Query"]!! as ViaductExtendedSchema.Object
    ...
    fun `Descriptions are descriptive`() {
        makeSchema(
            ...
            (this.types["E"]!! as ViaductExtendedSchema.Enum).value("A")!!.let { assertToStingContains("E.A", it, "EnumValue") }

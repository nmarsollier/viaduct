...
class ViaductAppPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        ...
        schemaProject.pluginManager.apply("test-schema")
        ...
        project.subprojects.forEach { sub ->
            sub.pluginManager.withPlugin("test-tenant") {
        ...
        project.gradle.projectsEvaluated {
            project.subprojects
                .filter { it.plugins.hasPlugin("test-tenant") }
                .forEach { sub ->
    ...
    private fun validateTenantIntegrity(project: Project) {
        project.gradle.projectsEvaluated {
            val tenantProjects = project.subprojects.filter {
                it.pluginManager.hasPlugin("test-tenant")
            }
    ...
    private fun Project.validateNestedSubProjects() {
        if (this.subprojects.isEmpty()) {
            ...
            this.subprojects.forEach {
                if (it.pluginManager.hasPlugin("test-tenant")) {

...
object JsonDomain {
    private class Impl(val internal: InternalContext, val resolveTypeName: ResolveTypeName) : Domain<String> {
        override val conv: Conv<String, IR.Value.Object> = Conv(
            forward = { str ->
                ...
                val conv = JsonConv(internal.schema, type)
    ...
    private fun interface ResolveTypeName {
        ...
        /** A [ResolveTypeName] that always resolves to the provided [type] */
        class Const(val type: Type<*>) : ResolveTypeName {
            override fun invoke(jsonString: String): String = type.name
        }
    ...
    fun <T : Input> forType(
        ...
        type: Type<T>
    ): Domain<String> = Impl(ctx.internal, ResolveTypeName.Const(type))
     ...
     * @see forType
     */
    operator fun invoke(ctx: ExecutionContext): Domain<String> = Impl(ctx.internal, ResolveTypeName.Parse)
}
